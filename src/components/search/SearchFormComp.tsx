...
const SearchFormComp: React.FC<IProps> = ({
  nodeItems,
  isDisabled = false,
  defaultValues,
}) => {
  ...
  const renderNodeItem = (items: INodeItem[]) => {
    return items.map(({ nodeType, id, label, xs, nodeOptions }, i) => {
      if (nodeType === "DATE_PICKER") {
        const { format = "YYYYMMDD" } = (nodeOptions || {}) as IDatePickerType;
        return (
                        ...
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              ...
                              checked={(() => {
                                if (getValues()[name]) {
                                  return getValues()[name].includes(opt.value);
                                }
                                return false;
                              })()}
                              onChange={(e) => {
            ...
            <Controller
              ...
              render={({ field: { value, onChange } }) => (
                <CategorysComp
                  ...
                  xs={xs}
                  categoryResetState={categoryResetState}
                  setCategoryResetState={setCategoryResetState}
                />
              )}
            />
          </Grid>
        ...
        return (
          <Grid key={i} item xs={xs || 12}>
            <Stack direction="row" spacing={5} alignItems="center">
              {label && (
                <Typography sx={{ wordBreak: "keep-all" }}>{label}</Typography>
              )}
  ...
  //카테고리리셋버튼 상태
  const [categoryResetState, setCategoryResetState] = useState<boolean>(false);
  ...
  const handleReset = () => {
    ...
    //상태 변경 후 겁색값 초기화
    setCategoryResetState(true);
    reset(defaultValues);
  };

  //searchbox 값을 쿼리전송을 위해 가공
